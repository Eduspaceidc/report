...
export async function getAdminList(sock, jid) {
  try {
    ...
    // Normalizar TODOS los IDs para incluir @s.whatsapp.net
...
export async function sendReport(sock, msg, args, config, sender, from) {
  try {
    ...
    // ✅ VERIFICAR SI EL USUARIO A EXPULSAR ES ADMINISTRADOR - CORREGIDO
    if (from.endsWith("@g.us")) {
      const admins = await getAdminList(sock, from);

      // Normalizar el usuario a expulsar de la misma manera
      const normalizedUserToKick = userToKick.includes('@s.whatsapp.net') ?
        userToKick : userToKick + '@s.whatsapp.net';

      // Comparación directa y consistente
      if (admins.includes(normalizedUserToKick)) {
        await sock.sendMessage(from, {
