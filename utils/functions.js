...
export async function handleMessage(sock, msg, config) {
  ...
  // 🆕 Función para enviar mensajes respondiendo al original
  ...
  if (body.startsWith('#kick')) {
    ...
    try {
      ...
      // Normalizar el sender para comparación consistente
      const normalizedSender = sender.includes('@s.whatsapp.net') ?
      ...
      if (!isSenderAdmin) {
        await sendReply("> ❌ Solo los administradores pueden usar el comando #kick.");
        return;
      }
      ...
      if (!targetUser) {
        await sendReply("> ❌ Debes mencionar o responder al usuario que será expulsado.");
        return;
      }

      // Evitar auto-expulsión o expulsar al bot - CORREGIDO
      const normalizedTargetUser = targetUser.includes('@s.whatsapp.net') ?
        targetUser : targetUser + '@s.whatsapp.net';

      if (normalizedTargetUser === normalizedSender || normalizedTargetUser === botNumber) {
        return;
      }

      // Verificar si el objetivo es administrador - CORREGIDO
      const isTargetAdmin = admins.includes(normalizedTargetUser);

      if (isTargetAdmin) {
        await sendReply("> ❌ No puedo eliminar a un administrador del grupo.");
        return;
      }
      ...
      await sock.sendMessage(from, {
        text: "❌ Error al intentar eliminar al usuario del grupo.",
      });
    }
    return;
  }
  ...
  if (from.endsWith("@g.us")) {
    ...
    const normalizedSender = sender.includes('@s.whatsapp.net') ?
      sender : sender + '@s.whatsapp.net';
    isAdmin = admins.includes(normalizedSender);
  }
  ...
  switch (command.toLowerCase()) {
    ...
    case "setg2":
    case "setrev":
      await handleSystemCommand(sock, from, sender, command, args, config, isOwner, msg);
      break;
    ...
    case "p":
      ...
      // 🕹️ Variable para controlar la velocidad (en milisegundos)
      // 👉 Este valor es el tiempo total que tarda en ir y volver la pelota
      // Ejemplo: 400 = rápido | 800 = medio | 1200 = lento
      const animationSpeed = 30; // 💥 ajusta esto libremente

      // ⏱️ Medir ping real (tiempo de envío del mensaje)
      const start = Date.now();
      const sent = await sock.sendMessage(from, { text: "🚨 ⚠️" }, { quoted: msg });
      const latency = Date.now() - start;

      // 🎾 Pelota: 6 pasos de ida + 6 de regreso (total 12 frames)
      const frames = [
        "🎯 Objetivo detectado...",
        "💥 Armando bomba...",
        "💣 3...",
      ...
      // ⚡ Calculamos cuánto debe durar cada frame según la velocidad deseada
      const frameDelay = animationSpeed / frames.length;

      // 🔁 Animación fluida
      for (const frame of frames) {
        await delay(frameDelay);
        await sock.sendMessage(from, { text: frame, edit: sent.key });
      }

      // 💥 Mensaje final con ping real
